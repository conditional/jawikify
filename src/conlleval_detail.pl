...
if (not $latex) {
   ...
   for $i (@sortedTypes) {
      ...
      printf "\n%-7s &  %6.2f\\%% (%3d/%3d) & %6.2f\\%% (%3d/%3d) & %6.2f \\\\",
             $i,$precision,$correctChunk{$i},$foundGuessed{$i},$recall,$correctChunk{$i},$foundCorrect{$i},$FB1;
   }
   print "\\hline\n";
   $precision = 0.0;
   $recall = 0;
   ...
   $FB1 = 2*$precision*$recall/($precision+$recall)
      if ($precision+$recall > 0);
   printf "Overall &  %6.2f\\%% (%3d/%3d) & %6.2f\\%% (%3d/%3d)& %6.2f \\\\\\hline\n",
          $precision,$correctChunk,$foundGuessed,$recall,$correctChunk,$foundCorrect,$FB1;
}
